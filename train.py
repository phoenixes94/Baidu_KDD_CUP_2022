...
def data_augment(X, y, p=0.8, alpha=0.5, beta=0.5):
    ...
    random_betas = paddle.to_tensor(np_betas, dtype="float32").reshape([-1, 1, 1, 1])
...
def train_and_evaluate(config, train_data, valid_data, test_data):
    ...
    _create_if_not_exist(config["checkpoints"])
    global_step = 0
    ...
    for epoch in range(config["epoch"]):
        for batch_x, batch_y in train_data_loader:
            ...
            opt.clear_gradients()
            global_step += 1
            if paddle.distributed.get_rank() == 0 and global_step % config["log_per_steps"] == 0:
                log.info("Step %s Train MSE-Loss: %s RMSE-Loss: %s" % (global_step, loss.numpy()[0], (paddle.sqrt(loss)).numpy()[0]))
        ...
        if paddle.distributed.get_rank() == 0:
            ...
            if best_score == valid_r['score']:
                patient = 0
                save_model(config["checkpoints"], model, steps=epoch)
            else:
                patient += 1
